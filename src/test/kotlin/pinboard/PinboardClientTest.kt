 ...
 */
@Ignore // this is an integration test that requires setup.
...
class PinboardClientTest {
    ...
    fun getRecentPosts() {
        ...
        (0..5).forEach({ i ->
            pinboardClient.addPost(someUrl + i, "description", "extended",
                    bookmark.tags, Date(), true, true, true)
        })
        val result = pinboardClient.getRecentPosts(tag = bookmark.tags)
    ...
    fun suggestTagsForPost() {
        val url = "https://www.washingtonpost.com/world/national-security/" +
    ...
    fun replaceTag() {
        addTestPost()
        assert(pinboardClient.replaceTag("wat", "what"), { "the old tag should've been renamed" })
    }
    */
}
